...
def invert_text_blink(text, uninvert_selected):
...
def prepare_smart_text(text: str, words: list[str], known_words: set[str], selected: int, blink_uninvert: bool = False) -> str:
    ...
    for i, segment in enumerate(static_segments):
        ...
        if i == selected and blink_uninvert:
            ...
            out_text += invert_text_blink(replacement)
"""